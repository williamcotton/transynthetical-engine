...
export const initialContext = `<div id='context'></div>`;
...
async function evaluator(
  ...
  try {
    if (solution.data) {
      ...
      const javascriptTarget = ts.transpileModule(solution.thunk, {
        compilerOptions: options,
      }).outputText;
      evaluated = await Function(`return ${javascriptTarget}()`)();
      ...
      const javascriptTarget = ts.transpileModule(solution.pthunk, {
        compilerOptions: options,
      }).outputText;
      ...
      evaluated = await Function(
        ...
        "React",
        `return ${javascriptTarget}(query, archiver, document)`
      )(
        query,
        archiver,
        iframeDoc ?? document,
